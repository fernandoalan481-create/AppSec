...
def analyze(html_body: str, base_url: str) -> List[Vulnerability]:
    ...
    for idx, form in enumerate(forms, start=1):
        ...
        if not action:
            findings.append(
                T.build(
                    ...
                    evidence=f'<form method="{method}"> [action ausente] — formulário #{idx}',
                )
            )
                ...
                findings.append(
                    T.build(
                        ...
                        evidence=(
                            ...
                            f"[scheme da página base: {base_scheme}]"
                        ),
                    )
                )
        ...
        if method == "post":
            ...
            if not (input_names & CSRF_TOKEN_NAMES):
                findings.append(
                    T.build(
                        T.FORM_NO_CSRF,
                        title=f"Formulário #{idx} sem token CSRF aparente",
                        evidence=f"Campos encontrados: {sorted(input_names) or '(nenhum)'} — formulário #{idx}",
                    )
                )
